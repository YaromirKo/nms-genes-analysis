---
title: "Analysis of genes with bootstrap and permutation (RD) Net [Sh, UBI] with mean"
...
corByGroup <- function(df, indices, dg, verbose=FALSE) {
  ...
  d.net <- df %>%
    ...
    summarise(Net_value = mean(log2(lm/rm), na.rm=TRUE), # median or mean 
...
rds_file <- file.path(main_dir, path_rds, paste0(
  "all_groups_boot_", formatter(R), "_", my.cores, "_cores_", DESIGN, "_Net_Sh_UBI_mean.rds"
))
...
groups_results <- paste0("./data/Net_Sh_UBI_mean_groups_result_", format(Sys.time(), '%d_%m_%Y'), ".xlsx")
...
for (item in unique(data$Segment)) {
  ...
  name <- paste0(item, " Networks Sh UBI mean")
...
myname <- "corr_groups_Net_Sh_UBI_mean_"
